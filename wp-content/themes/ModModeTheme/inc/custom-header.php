...
function upbootwp_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'upbootwp_custom_header_args', array(
		'default-image'          => get_template_directory_uri() . '/images/1770x600&text=need+1770x600+lifestyle+image.gif',
...
?>
	...
	.appearance_page_custom-header #headimg {
		...
		<?php
		if ( ! empty( $header_image ) ) {
			echo 'background: url(' . esc_url( $header_image ) . ') no-repeat scroll top; background-size: 1400px 600px;';
		} ?>
		padding: 0 20px;
	}
	#headimg .home-link {
...
function upbootwp_admin_header_image() {
	...
	$header_image = get_header_image();
?>
	<div id="headimg" style="background: url(<?php header_image(); ?>) no-repeat scroll top; background-size: 1400px 600px;">
